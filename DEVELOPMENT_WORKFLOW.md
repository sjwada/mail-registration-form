...
### Git のみのデプロイ（Clasp 使用不可の場合）

```bash
# ビルド + Git add/commit/push
npm run git-push
```

**注意**: `git-push` コマンドは対話的にコミットメッセージを入力します。

### Clasp アカウント管理

Clasp は別の Google アカウントでログインしている場合、以下の手順でアカウントを切り替えます:
...
これにより、本番環境の URL はそのままで、最新のコードが反映されます。
...
#### 原因 2: 間違ったデプロイメントを見ている

テストデプロイメントの URL が正しいか確認してください。

#### 原因 3: clasp push が失敗している
...
`Config.js` には機密情報（スプレッドシート ID、トークン等）が含まれるため、慎重に管理します。

### 4. バックアップ

重要な変更の前には、必ず Git にコミットしてバックアップを取ります。
...
| 変数名           | 説明                | 例                               |
| ---------------- | ------------------- | -------------------------------- |
| SPREADSHEET_ID   | スプレッドシート ID | `1Erl9e69j...`                   |
